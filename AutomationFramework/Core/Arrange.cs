...
namespace EasyAutomation.AutomationFramework.Core
{
    ...
    {
        /* var task = Task.Factory.StartNew<T, ElementInformation>(() =>
            {
                string name = "???";
                string automationId = "???";
                string localizedControlType = "???";

                Process[] p = Process.GetProcessesByName("ExampleApplication");
                ...
                return new Tuple<T, ElementInformation>(default(T), new ElementInformation(name, automationId, localizedControlType));
*/
